...
gulp.task('build',
 gulp.series(clean, gulp.parallel(pages, sass, javascript, images, copy), styleGuide, article));
...
function article(done) {
  sherpa('src/article/index.md', {
    ...
    template: 'src/article/template.html'
  }, done);
}
...
function watch() {
  ...
  gulp.watch('src/article/**', gulp.series(article, browser.reload));
}
